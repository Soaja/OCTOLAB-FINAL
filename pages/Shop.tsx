...
export const Shop: React.FC<ShopProps> = ({ onProductClick }) => {
  return (
    <div className="pt-32 min-h-screen bg-transparent relative">
      <ModernBackground />

      {/* HEADER SECTION - REIMAGINED */}
      <div className="max-w-[1400px] mx-auto px-6 mb-40 relative z-10">
        <motion.div 
        ...
        >
           ...
           <h1 className="text-7xl md:text-9xl font-bold tracking-tighter text-[#0B0B0C] mb-8 leading-[0.85]">
             Research <br/>
             <span className="text-neutral-300">Library.</span>
           </h1>
           
           <p className="text-xl md:text-2xl text-neutral-500 font-light max-w-xl leading-relaxed mt-4">
      ...
      <div className="max-w-[1400px] mx-auto px-6 pb-32 relative z-10 flex flex-col gap-40">
             ...
             return (
              ...
              >
                <div className={`flex flex-col lg:flex-row items-center gap-16 lg:gap-32 ${!isEven ? 'lg:flex-row-reverse' : ''}`}>
                   ...
                   >
                      <div className="aspect-square relative rounded-[3rem] bg-[#F5F5F7] overflow-hidden flex items-center justify-center p-16 shadow-inner">
                   ...
                   <div className="flex-1 w-full flex flex-col items-start text-left">
                      ...
                      <h2 
                        onClick={() => onProductClick(product)}
                        className="text-6xl md:text-8xl font-bold text-[#0B0B0C] tracking-tighter mb-6 cursor-pointer hover:text-neutral-700 transition-colors leading-[0.9]"
                      >
                        {product.name}
                      </h2>
                      
                      {/* Subtitle / Chem Name */}
                      <p className="text-xl font-mono text-neutral-400 mb-8 tracking-tight">
                        {product.subtitle}
                      </p>
                      
                      {/* Description */}
                      <p className="text-lg text-neutral-600 leading-relaxed max-w-md mb-12 font-medium">
                        {product.description}
                      </p>

                      {/* Specs Grid - Minimalist */}
                      <div className="grid grid-cols-2 gap-y-8 gap-x-12 mb-14 w-full max-w-md border-t border-neutral-100 pt-8">
                         <div className="flex flex-col gap-2">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">Volume</span>
                            <span className="text-xl font-medium text-neutral-900">{product.volume}</span>
                         </div>
                         <div className="flex flex-col gap-2">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">Purity (HPLC)</span>
                            <span className="text-xl font-medium text-neutral-900">&gt;99.9%</span>
                         </div>
                         <div className="flex flex-col gap-2">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">Dosage</span>
                            <span className="text-xl font-medium text-neutral-900">{product.dosage}</span>
                         </div>
                         <div className="flex flex-col gap-2">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400">Temp</span>
                            <span className="text-xl font-medium text-neutral-900">-20°C</span>
                         </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-8 w-full">
                         <div className="flex flex-col">
                            ...
                            <span className="text-5xl font-light tracking-tight text-[#0B0B0C]">
                         ...
                         <Button 
                           onClick={() => onProductClick(product)}
                           className="flex-1 bg-[#0B0B0C] text-white hover:bg-neutral-800 h-16 rounded-full text-lg shadow-2xl shadow-neutral-200 group relative overflow-hidden"
                         >
